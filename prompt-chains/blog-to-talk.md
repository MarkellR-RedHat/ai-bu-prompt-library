...
- **Related tool:** Once this chain produces your talk outline, use [slide-outliner](https://github.com/MarkellR-RedHat/slide-outliner) to build the slide deck and [cfp-generator](https://github.com/MarkellR-RedHat/cfp-generator) to polish the CFP submission. If you use `claude-commands`, the `/blog-from-pr` slash command can generate the blog post that feeds into this chain, and the `/demo-prep` slash command can build the live demo portion of the resulting talk.
