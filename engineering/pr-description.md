...
- **Suite connection:** After your PR merges, [ai-bu-shipped-digest](https://github.com/MarkellR-RedHat/ai-bu-shipped-digest) can roll it into a weekly summary of everything the team shipped. If you use `ai-bu-claude-commands`, the `/changelog` and `/release-notes` slash commands can generate release-facing summaries from the same diff.
