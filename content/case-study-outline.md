...
**What you get:** "The customer faced significant challenges with their AI infrastructure. After implementing our solution, they saw improved performance and significant cost savings. The deployment was smooth and the team was impressed with the results. The customer looks forward to expanding their use of the platform in the future." Four sentences, zero numbers, no architecture, no named product. You cannot tell whether this is about inference serving, a database migration, or a CRM rollout.

**This prompt produces:** A structured, 11-section outline with quantified before/after metrics (GPU utilization from 40% to 82%, inference costs reduced by 60%, model deployment time from 2-3 days to 45 minutes), explicit flags for any data gaps ([NEEDS DATA]) or unverified claims ([NEEDS VALIDATION]), a solution architecture section naming llm-d on OpenShift AI with disaggregated prefill/decode and KV-cache-aware routing, and a key quote placeholder that sounds like a real person ("We went from three-day deployments to 45 minutes") rather than a press release. The outline is designed to be shared directly with the customer contact for accuracy review before you invest time in drafting.
